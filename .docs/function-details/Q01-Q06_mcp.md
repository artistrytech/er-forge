...
```json
{
  ...
  "lastAccess": { "at": "2026-09-04T09:31:22Z", "tool": "erd_list_tables", "era": "modern", "write": false }
}
```
...
- **ツール呼び出しに限らず、あらゆる MCP リクエストで記録する。** 「繋がっているのか」
  「どちらの世代（§3.0）で来ているのか」は接続の切り分けで最初に知りたいことであり、
  ツールを1回も呼ばずに終わる接続（`initialize` だけ通って切れる等）もあるためである。
  `tools/call` だけはメソッド名よりツール名のほうが役に立つので、そちらを記録する。
...
2. **`meta` 配下のみ**にパッチを当てる。machine-owned のフィールドは読んだ値をそのまま使う
   （`Table.withMeta` — 型の上でも `schema` に触れない）
3. 引数に machine-owned のキーが混ざっていたら**黙って捨てず `isError`** にする
   （「無視した」と伝えないと、モデルは書けたつもりで先へ進む）
4. `TableService.put(dataDir, id, Table, baseHash, force=false)` に渡す。
   このためにモデルを直接受ける入口を `TableService` に足した（JSON 版はそれを呼ぶだけ）。
   JSON に直してパースし直す往復を省くためで、検証・正規化・`index.js` 再生成は JSON 版と同一経路
...
- **`layout: "keep"`** は既存ノードの座標を保ち、新規分だけを**既存の最下段の下に ID 順で左から**並べる
  （幅 1600px で折り返し）。ELK を全体に掛けると既存の配置が崩れるため、keep では ELK を使わない。
- ノードの寸法は**論理名と物理名の長いほうから概算**する（`14 × 文字数 + 48`、180〜420px。高さは既定値）。
  ページに `w` の明示があればそれを使う。**ビューア側の実測値には依存しない**（MCP はブラウザを持たないため）。
  多少ずれても、人が GUI で `fitView` すれば済む。
- ELK の結果は原点 (0,0) 基準なので、80px の余白を足してページに置く。
- **決定論的であること**をテストで固定する（同じページに `erd_auto_layout` を2回掛けてもファイルが変わらない）。
  ELK は入力順に対して決定論的なので、ノード集合を ID 順に固定してから渡す。
- 実装は `McpDiagramTools`。`McpWriteTools` が名前で振り分け、バックアップ（Q-06）は両方に共通で掛かる。
...
| 設定スニペット | **実ポートとトークンを埋め込んだ設定**を表示して [コピー]。A-11 が等価な CLI コマンドを常に見せているのと同じ思想。**トークンを実物で埋められるのは発行直後だけ**なので、それ以外は `<発行したトークン>` のプレースホルダを出す（伏字を貼らせない）。ここを「いつでも実物が見られる」ようにすると、画面を開くだけでトークンが漏れる経路になる |
| クライアントの切替 | **タブで Claude Code / Codex / Cursor / GitHub Copilot を切り替える**。形はツールごとに違い、**貼り付け先のファイルも、トップレベルのキーも、ヘッダの書き方も揃っていない**ため、1つの例だけでは他のツールの利用者が必ず詰まる。貼り付け先のパスも一緒に出す。下表は各ツールの公式ドキュメントに合わせた写しで、変えるときは出典を確認する |

| クライアント | 貼り付け先 | 形 |
|---|---|---|
| Claude Code | `.mcp.json` | `mcpServers` + `type: "http"` + `headers` |
| Codex | `~/.codex/config.toml` | **TOML**。`[mcp_servers.<id>]` + `url` + `http_headers` |
| Cursor | `.cursor/mcp.json` | `mcpServers` + `url`（`url` があれば HTTP 扱いで `type` は不要） |
| GitHub Copilot (VS Code) | `.vscode/mcp.json` | **`servers`**（`mcpServers` ではない）+ `type: "http"` |
| 最終アクセス | 直近のリクエストの時刻・ツール名（またはメソッド名）・**世代（modern / legacy）**・書き込みの有無。繋がっているかどうかを画面だけで確かめられるようにする |
| 注意書き | 「サーバーを停止すると AI から見えなくなる」「**AI クライアントは起動時に接続するので、サーバーを後から起動した場合や設定を変えた場合はクライアント側で接続し直す**」「ポートが変わったら貼り直す」 |
...
| T-11 | Q-06 | 最初の書き込み前に `.local/workspace-<id>/backup/<ts>/` が1世代でき、直後の書き込みでは増えない。バックアップに失敗しても書き込みは止めない（最後の砦は Git） |
| T-12 | 部分更新 | `erd_set_table_meta` で省略した項目は変わらず、`null` を渡した項目だけ消える。物理情報（`columns[].type`）は無傷 |
