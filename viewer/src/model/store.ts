...
export interface AppState {
  ...
  recentTables: string[];
  /**
   * ページ管理ダイアログ（ページの追加・改名・並び替え・削除。左パネルのペンで開く）を
   * 出しているか。
   *
   * これらは**即時にファイルへ書かれる**（ER図の配置編集のような「保存」を挟まない）ため、
   * ER図の編集セッションとは別の導線に分ける。ER編集中は開けず、逆に開いている間は
   * ER図・テーブルの編集を開始できない（どちらの意味で編集中なのかを曖昧にしない）。
   * 開閉の状態をここに置くのは、左パネルが ER用・テーブル用の2つ同時にマウントされており、
   * ダイアログの実体は App が1つだけ出すため（パネル側に持たせると二重になる）。
   */
